var data = "";
const pdf = ghB("news_pdf");
const s_btn = ghB("news_btn");

ghB("hero_img_upload").on("change", async (e) => {
    data = await toBase64(e.target.files[0]);
    ghB("hero_file_preview").src = data;
});

ghB("news_form").on("submit", admin_sendNews);

ghB("hero_img_upload_submit").click(async () => {
    if (data === "") return alert("Kein Bild hochgeladen.");
    post("/post/changeHeroImg", { base64: data });
});

ghB("submit-make-admin").click(async () => {
    let username = ghB("make-admin").val();
...
ghB("submit-delete-admin").click(async () => {
    let username = ghB("delete-admin").val();
    if (username === "") return alert("Bitte gib einen gültigen Benutzernamen ein.");
...
ghB("gallery_img_upload_submit").click(async () => {
    let title = ghB("gallery_title").val(),
        subtitle = ghB("gallery_subtitle").val(),
        author = ghB("username").val();
    if (title === "") return alert("Fülle alle Felder aus.");
    let img = { arr: [], vid: [] };
    let files = ghB("gallery_img_upload").files;
...
ghB("gallery_update_submit").click(async (e) => {
    e.preventDefault();

    const title = ghB("gallery_update_title");

    if (title.valIsEmpty()) return alert("Fülle alle Felder aus.");

    const data = { img: [], vid: [] };

    const files = ghB("gallery_update").files;
...
ghB("merge_blogs_btn").click(async () => {
    ...
    const res = await post("/post/mergeBlogs", {
        title: ghB("merge_blogs_title").val(),
        description: ghB("merge_blogs_desc").val(),
        base64: await ghB("merge_blogs_basic_img").getImgBase64(),
        alt: ghB("merge_blogs_basic_alt").val(),
        number: Number(ghB("merge_blogs").val()),
        team,
    });
...
ghB("team-m-submit").click(async () => {
    const leitsatz = ghB("team-m-leitsatz");
    const beschreibung = ghB("team-m-beschreibung");
    ...
    const base64 = await ghB("team-m-img").getImgBase64();
...
ghB("delete_blog_btn").click(async () => {
    const title = ghB("delete_blog");
...
ghB("delete_event_btn").click(async () => {
    const title = ghB("delete_event");
...
pdf.click(() => toggleDivs(pdf, ghB("show_pdf"), ghB("show_img")));
s_btn.click(() => toggleDivs(s_btn, ghB("show_btn_menu"), ghB("show_nothing")));
...
async function admin_sendNews(e) {
    ...
    let picture = "";
    try {
        picture = await toBase64(ghB("news_img_file").file());
    } catch (err) {
        warnLog("No image uploaded");
    }

    let res = await post("/post/submitNews", {
        text: ghB("news_text").val(),
        img: picture,
        img_alt: ghB("news_img_alt").val(),
        img_pos: ghB("news_img_pos").val(),
        btn: ghB("news_btn").checked.toString(),
        btn_text: ghB("news_btn_text").val(),
        btn_link: ghB("news_btn_link").val(),
        pdf: ghB("news_pdf").checked.toString(),
        pdf_src: ghB("news_pdf_src").val(),
    });
...
ghB("team-m-add").click(async () => {
    const username = ghB("team-m-username");
    const job = ghB("team-m-job");
    const motivation = ghB("team-m-motivation");
...
ghB("team-m-remove").click(async () => {
    const username = ghB("team-m-username-remove");
...
ghB("event_submit").click(async (e) => {
    e.preventDefault();
    const title = ghB("event_title");
    const date = ghB("event_date");
