on(gM, "DOMContentLoaded", () => {
    setTimeout(() => {
        const content = ghB("content");
        const edit_btn = ghB("edit-current-element");
        const delete_btn = ghB("delete-current-element");
        const hero_title = ghC(".hero-title");
        const hero_subtitle = ghC(".hero-subtitle");
        const hero_img = ghC(".hero-img");
        const json = {
            html: null,
            hero: {
        ...
        ghB("add-picture").click(() => addImg("img-only"));

        ghB("add-picture-text").click(() => addImg("left"));

        ghB("add-text-picture").click(() => addImg("right"));
        ...
        ghB("add-multiple-pictures").click(() => {
            const input = createElm("input");
            input.type = "file";
            input.accept = "image/*";
            input.multiple = true;
            input.on("change", async (e) => {
                ...
                for (let i = 0; i < fileArray.length; i++) {
                    scriptContent += `ghA(".${fileArray[i].id}"),`;
                }
        ...
        ghB("${nextBtnId}").click(${funcNextName});
        ghB("${prevBtnId}").click(${funcPrevName});
        ...
        ghB("done").click(async () => {
            ...
            ghA(".blog_carousel img").forEach((e) => e.removeClass("active"));

            ghA(".main *").forEach((element) => {
        ...
        function jsonToElm(options) {
            const element = gM.createElement(options.tagName);
            ...
            for (let child of options.children) {
                if (child.tagName === "___text___") {
                    element.appendChild(gM.createTextNode(child.content));
                    continue;
                }
