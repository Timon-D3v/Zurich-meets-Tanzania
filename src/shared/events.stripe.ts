...
export async function paymentIntentSucceeded(event: any): Promise<void> {
        ...
        if (result.data.length === 0) {
            const createMemberResult = await createMember(userId, subscription.id, customerId, subscription.status, subscription.items.data[0].current_period_start * 1000, subscription.items.data[0].current_period_end * 1000, subscription.start_date * 1000);

            if (createMemberResult.error) {
                throw new Error(createMemberResult.error);
            }
        } else {
            const updateMemberResult = await updateMember(result.data[0].memberId, userId, subscription.id, customerId, subscription.status, subscription.items.data[0].current_period_start * 1000, subscription.items.data[0].current_period_end * 1000, subscription.start_date * 1000);
